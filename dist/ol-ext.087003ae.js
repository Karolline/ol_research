...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"dist/ol-ext.js":[function(require,module,exports) {
...
ol.ext.Ajax.prototype.send = function (url, data, options) {
  ...
  if (encode) url = encodeURI(url); // Parameters
...
ol.ext.element.scrollDiv = function (elt, options) {
  ...
  var scroll = options.vertical ? 'scrollTop' : 'scrollLeft'; // Prevent image dragging
  ...
  if (options.mousewheel && !elt.classList.contains('ol-touch')) {
    ol.ext.element.addListener(elt, ['mousewheel', 'DOMMouseScroll', 'onmousewheel'], function (e) {
...
ol.ext.getMapCanvas = function (map) {
  ...
  if (!canvas && map.getViewport().querySelector('.ol-layers')) {
    ...
    map.getViewport().querySelector('.ol-layers').after(canvas); // Clear before new compose
...
ol.control.CanvasBase = function (options) {
  if (!options) options = {}; // Define a style to draw on the canvas
...
ol.control.CanvasBase.prototype.setMap = function (map) {
  ...
  if (map) {
    this._listener = map.on('postcompose', this._draw.bind(this)); // Get a canvas layer on top of the map
  }
};
/** Get canvas overlay
 */
...
ol.control.CanvasBase.prototype._draw = function ()
/* e */
{
...
ol.control.SelectBase = function (options) {
  ...
  element.appendChild(options.content); // OK button
...
ol.control.SelectBase.prototype.setFeatures = function (features) {
  if (features instanceof ol.Collection) this._features = features;else this._features = null;
};
...
ol.control.Search = function (options) {
  var self = this;
  if (!options) options = {};
  if (options.typing == undefined) options.typing = 300; // Class name for history

  this._classname = options.className || 'search';
  var classNames = (options.className || '') + ' ol-search' + (options.target ? '' : ' ol-unselectable ol-control ol-collapsed');
  ...
  var doSearch = function doSearch(e) {
    ...
    var val = input.value; // move up/down
  ...
  element.appendChild(input); // Reverse geocode
  ...
  if (typeof options.autocomplete == 'function') this.autocomplete = options.autocomplete; // Options

  this.set('copy', options.copy);
  this.set('minLength', options.minLength || 1);
  this.set('maxItems', options.maxItems || 10);
  this.set('maxHistory', options.maxHistory || options.maxItems || 10); // History
...
ol.control.Search.prototype.reverseGeocode = function ()
/*coord, cback*/
...
ol.control.Search.prototype._handleSelect = function (f, reverse) {
  if (!f) return; // Save input in history

  var hist = this.get('history'); // Prevent error on stringify
  ...
  this.saveHistory(); // Select feature

  this.select(f, reverse); //this.drawList_();
};
/** Current history */
  ...
  for (var i = 0; i < max; i++) {
    if (auto[i]) {
      if (!i || !self.equalFeatures(auto[i], auto[i - 1])) {
        ...
        if (title instanceof Element) li.appendChild(title);else li.innerHTML = title;
        ul.appendChild(li);
      }
    }
  }
...
ol.control.Search.prototype.equalFeatures = function ()
/* f1, f2 */
{
  return false;
};
...
ol.control.SearchJSON = function (options) {
  ...
  ol.control.Search.call(this, options); // Handle Mix Content Warning
  ...
  }.bind(this)); // Overwrite handleResponse
...
ol.control.SearchPhoton.prototype.getTitle = function (f) {
  var p = f.properties;
  return (p.housenumber || "") + " " + (p.street || p.name || "") + "<i>" + " " + (p.postcode || "") + " " + (p.city || "") + " (" + p.country + ")</i>";
};
/** 
 * @param {string} s the search string
 * @return {Object} request data (as key:value)
...
ol.control.SearchPhoton.prototype.equalFeatures = function (f1, f2) {
  return this.getTitle(f1) === this.getTitle(f2) && f1.geometry.coordinates[0] === f2.geometry.coordinates[0] && f1.geometry.coordinates[1] === f2.geometry.coordinates[1];
};
/** A ligne has been clicked in the menu > dispatch event
*	@param {any} f the feature, as passed in the autocomplete
*	@api
...
ol.control.SearchPhoton.prototype.select = function (f) {
  var c = f.geometry.coordinates; // Add coordinate to the event
...
ol.control.SearchPhoton.prototype.reverseGeocode = function (coord, cback) {
  this.ajax(this.get('url').replace('/api/', '/reverse/').replace('/search/', '/reverse/'), this.reverseData(coord), function (resp) {
    ...
    if (cback) {
      ...
      this._handleSelect(resp[0], true); // this.setInput('', true);
...
ol.control.SearchGeoportail = function (options) {
  ...
  ol.control.SearchJSON.call(this, options);
  this.set('type', options.type || 'StreetAddress,PositionOfInterest'); // Authentication
  ...
  this.ajax(this.get('url').replace('ols/apis/completion', 'geoportail/ols'), {
    ...
    if (xml) {
      ...
      if (cback) {
        ...
        this._handleSelect(f, true); // this.setInput('', true);
        // this.drawList_();
...
ol.control.SearchGeoportail.prototype.handleResponse = function (response) {
  ...
  if (this.get('type') === 'Commune') {
    for (var i = features.length - 1; i >= 0; i--) {
      if (features[i].kind && (features[i].classification > 5 || features[i].kind == "Département")) {
        features.splice(i, 1);
      }
    }
  }
...
ol.control.SearchGeoportail.prototype.select = function (f) {
  if (f.x || f.y) {
    var c = [Number(f.x), Number(f.y)]; // Add coordinate to the event
...
ol.control.SearchGeoportail.prototype.searchCommune = function (f, cback) {
  var request = '<?xml version="1.0" encoding="UTF-8"?>' + '<XLS xmlns:xls="http://www.opengis.net/xls" xmlns:gml="http://www.opengis.net/gml" xmlns="http://www.opengis.net/xls" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.2" xsi:schemaLocation="http://www.opengis.net/xls http://schemas.opengis.net/ols/1.2/olsAll.xsd">' + '<RequestHeader/>' + '<Request requestID="1" version="1.2" methodName="LocationUtilityService">' + '<GeocodeRequest returnFreeForm="false">' + '<Address countryCode="PositionOfInterest">' + '<freeFormAddress>' + f.fulltext + '+</freeFormAddress>' + '</Address>' + '</GeocodeRequest>' + '</Request>' + '</XLS>'; // Search 
  ...
  this.ajax(this.get('url').replace('ols/apis/completion', 'geoportail/ols'), {
    ...
    if (xml) {
      ...
      if (f.x || f.y) {
        if (cback) cback.call(this, [f]);else this._handleSelect(f);
      }
    }
  }.bind(this), {
    dataType: 'XML'
...
ol.control.LayerSwitcher = function (options) {
  ...
  this._layers = []; // displayInLayerSwitcher
  ...
  if (options.target) {
    ...
    if (options.collapsed !== false) element.classList.add('ol-collapsed');else element.classList.add('ol-forceopen');
...
ol.control.LayerSwitcher.prototype.setMap = function (map) {
  ...
  this._listener = null; // Get change (new layer added or removed)
...
ol.control.LayerSwitcher.prototype.overflow = function (dir) {
  if (this.button) {
    ...
    var dh = this.button.offsetTop + ol.ext.element.outerHeight(this.button); //var dh = this.button.position().top + this.button.outerHeight(true);
...
ol.control.LayerSwitcher.prototype.viewChange = function () {
  this.panel_.querySelectorAll('li').forEach(function (li) {
    ...
    if (l) {
      if (this.testLayerVisibility(l)) li.classList.remove('ol-layer-hidden');else li.classList.add('ol-layer-hidden');
    }
  }.bind(this));
};
/**
...
ol.control.LayerSwitcher.prototype.drawPanel = function () {
  ...
  var self = this; // Multiple event simultaneously / draw once => put drawing in the event queue
...
ol.control.LayerSwitcher.prototype.drawPanel_ = function () {
  if (--this.dcount || this.dragging_) return; // Remove existing layers
...
ol.control.LayerSwitcher.prototype.dragOrdering_ = function (e) {
  ...
  var target, dragElt;
  var layer, group;
  elt.parentNode.classList.add('drag'); // Stop ordering
  ...
  function stop() {
    if (target) {
      ...
      if (drop && target) {
        var collection;
        if (group) collection = group.getLayers();else collection = self.getMap().getLayers();
        var layers = collection.getArray(); // Switch layers
  ...
  function move(e) {
    ...
    if (start && Math.abs(pageY0 - pageY) > 2) {
      ...
      target = false;
      group = self._getLayerForLI(elt.parentNode.parentNode); // Ghost div
    ...
    if (!start) {
      ...
      if (li && li.parentNode.classList.contains('drag') && li !== elt) {
        target = self._getLayerForLI(li); // Don't mix layer level
        ...
        target = false;
        if (li === elt) ol.ext.element.hide(dragElt);else ol.ext.element.show(dragElt);
      }
...
ol.control.LayerSwitcher.prototype.dragOpacity_ = function (e) {
  e.stopPropagation();
  e.preventDefault();
  var self = this; // Register start params
  ...
  self.dragging_ = true; // stop dragging
...
ol.control.LayerSwitcher.prototype.drawList = function (ul, collection) {
  var self = this;
  var layers = collection.getArray(); // Change layer visibility
  ...
  function zoomExtent(e) {
    ...
    if (self.onextent) self.onextent(l);else self.getMap().getView().fit(l.getExtent(), self.getMap().getSize());
    self.dispatchEvent({
      type: "extent",
      layer: l
    });
  ...
  function removeLayer(e) {
    ...
    var layer,
        group = self._getLayerForLI(li); // Remove the layer from a group or from a map
  ...
  for (var i = layers.length - 1; i >= 0; i--) {
    ...
    var d = ol.ext.element.create('DIV', {
      ...
      // + (this.testLayerVisibility(layer) ? '' : ' ol-layer-hidden'),
      parent: li
    }); // Visibility
    ...
    } else if (layer instanceof ol.layer.Vector) li.classList.add('ol-layer-vector');else if (layer instanceof ol.layer.VectorTile) li.classList.add('ol-layer-vector');else if (layer instanceof ol.layer.Tile) li.classList.add('ol-layer-tile');else if (layer instanceof ol.layer.Image) li.classList.add('ol-layer-image');else if (layer instanceof ol.layer.Heatmap) li.classList.add('ol-layer-heatmap'); // Dispatch a dralist event to allow customisation
...
ol.control.Bar.prototype.setMap = function (map) {
  ...
  for (var i = 0; i < this.controls_.length; i++) {
    var c = this.controls_[i]; // map.addControl(c);
...
ol.control.Button = function (options) {
  ...
  element.appendChild(bt); // Try to get a title in the button content
...
ol.control.Button.prototype.setVisible = function (val) {
  if (val) ol.ext.element.show(this.element);else ol.ext.element.hide(this.element);
};
/**
...
ol.control.CanvasAttribution = function (options) {
  if (!options) options = {};
  ol.control.Attribution.call(this, options); // Draw in canvas

  this.setCanvas(!!options.canvas); // Get style options
...
ol.control.CanvasAttribution.prototype.drawAttribution_ = function (e) {
  ...
  if (!ctx) return;
  var text = "";
  Array.prototype.slice.call(this.element.querySelectorAll('li')).filter(function (el) {
  ...
  ctx.save();
  ctx.scale(ratio, ratio); // Position
  ...
  var left = w / 2 + this.element.querySelectorAll('button')[0].clientWidth; // Draw scale text
...
ol.control.CanvasScaleLine = function (options) {
  ...
  this.scaleHeight_ = 6; // Get style options
...
ol.control.CanvasScaleLine.prototype.setMap = function (map) {
  ...
  this._listener = null;
  ol.control.ScaleLine.prototype.setMap.call(this, map);
  if (oldmap) oldmap.renderSync(); // Add postcompose on the map
...
ol.control.CanvasScaleLine.prototype.setStyle = function (style) {
  ...
  this.fontStrokeWidth_ = stroke ? stroke.getWidth() : 3;
  this.fontFillStyle_ = fill ? ol.color.asString(fill.getColor()) : this.strokeStyle_; // refresh
...
ol.control.CanvasScaleLine.prototype.drawScale_ = function (e) {
  ...
  var ctx = this.getContext(e);
  if (!ctx) return; // Get size of the scale div
  ...
  var ratio = e.frameState.pixelRatio;
  ctx.save();
  ctx.scale(ratio, ratio); // On top

  position.top += this.element.clientHeight - this.scaleHeight_; // Draw scale text
...
ol.control.CanvasTitle.prototype.setStyle = function (style) {
  ol.control.CanvasBase.prototype.setStyle.call(this, style); // Element style
...
ol.control.CenterPosition.prototype._draw = function (e) {
  if (!this.getVisible() || !this.getMap()) return; // Coordinate
...
ol.control.Compass = function (options) {
  ...
  this.set('rotateVithView', options.rotateWithView !== false); // The image
  ...
  for (var i = 0; i < 8; i++) {
    this.da_[i] = [Math.cos(Math.PI * i / 8), Math.sin(Math.PI * i / 8)];
  }
};
...
ol.control.Compass.prototype._draw = function (e) {
  var ctx = this.getContext(e);
  if (!ctx) return;
  var canvas = ctx.canvas; // Retina device
...
ol.control.Dialog.prototype.setContent = function (options) {
  ...
  ol.ext.element.setHTML(form.querySelector('.ol-content'), options.content || ''); // Title
...
ol.control.EditBar = function (options) {
  options = options || {};
  options.interactions = options.interactions || {}; // New bar
  ...
  this._source = options.source; // Add buttons / interaction
...
ol.control.EditBar.prototype._setSelectInteraction = function (options) {
  var self = this; // Sub bar

  var sbar = new ol.control.Bar();
  var selectCtrl; // Delete button
    ...
    if (options.interactions.DrawLine instanceof ol.interaction.Draw) {
      ...
      this._interactions.DrawLine = new ol.interaction.Draw({
        ...
        geometryFunction: function geometryFunction(coordinates, geometry) {
          if (geometry) geometry.setCoordinates(coordinates);else geometry = new ol.geom.LineString(coordinates);
    ...
    if (options.interactions.DrawPolygon instanceof ol.interaction.Draw) {
      ...
      this._interactions.DrawPolygon = new ol.interaction.Draw({
        ...
        geometryFunction: function geometryFunction(coordinates, geometry) {
          ...
          if (geometry) geometry.setCoordinates([coordinates[0].concat([coordinates[0][0]])]);else geometry = new ol.geom.Polygon(coordinates);
          return geometry;
        }
      });
    }
...
ol.control.EditBar.prototype._setModifyInteraction = function (options) {
  ...
  if (options.interactions.ModifySelect !== false && options.interactions.Select !== false) {
    ...
    if (this.getMap()) this.getMap().addInteraction(this._interactions.ModifySelect); // Activate with select
...
ol.control.Gauge.prototype.setTitle = function (title) {
  ...
  if (!title) this.title_.display = 'none';else this.title_.display = '';
};
...
ol.control.GeoBookmark = function (options) {
  ...
  if (options.target) {
    ...
    element.className = (options.className || "ol-bookmark") + " ol-unselectable ol-control ol-collapsed";
    element.addEventListener("mouseleave", function () {
      if (input !== document.activeElement) {
  ...
  this.set("editable", options.editable !== false); // Set default bmark
...
ol.control.GeoBookmark.prototype.addBookmark = function (name, position, zoom, permanent) {
  if (!name) return;
  var bmark = this.getBookmarks(); // Don't override permanent bookmark

  if (bmark[name] && bmark[name].permanent) return; // Create or override
...
ol.control.GeolocationBar = function (options) {
  ...
  var element = this.element; // Geolocation draw interaction
  ...
  this._geolocBt.setActive(false); // Buttons
...
ol.control.Globe = function (opt_options) {
  ...
  this.set('follow', options.follow || false); // Cache extent
...
ol.control.Globe.prototype.setMap = function (map) {
  if (this._listener) ol.Observable.unByKey(this._listener);
  this._listener = null;
  ol.control.Control.prototype.setMap.call(this, map); // Get change (new layer added or removed)
...
ol.control.Globe.prototype.show = function (b) {
  if (b !== false) this.element.classList.remove("ol-collapsed");else this.element.classList.add("ol-collapsed");
...
ol.control.Globe.prototype.setPosition = function (align) {
  if (/top/.test(align)) this.element.classList.add("ol-control-top");else this.element.classList.remove("ol-control-top");
  if (/right/.test(align)) this.element.classList.add("ol-control-right");else this.element.classList.remove("ol-control-right");
};
...
ol.control.GridReference = function (options) {
  ...
  if (typeof options.indexTitle == 'function') this.indexTitle = options.indexTitle; // Set index using the source

  this.source_ = options.source;

  if (options.source) {
    this.setIndex(options.source.getFeatures(), options); // reload on ready
...
ol.control.GridReference.prototype.setIndex = function (features) {
  ...
  var searchKeyupFunction = function searchKeyupFunction() {
    var v = this.value.replace(/^\*/, ''); // console.log(v)
    ...
    ul.querySelectorAll('li').forEach(function (li) {
      if (li.classList.contains('ol-title')) {
        ...
        if (r.test(li.querySelector('.ol-name').textContent)) li.style.display = '';else li.style.display = 'none';
      }
    });
    ul.querySelectorAll("li.ol-title").forEach(function (li) {
      ...
      if (nextVisible.length && !nextVisible.classList.contains('ol-title')) li.style.display = '';else li.style.display = 'none';
    });
  };
...
ol.control.GridReference.prototype._draw = function (e) {
  ...
  ctx.scale(ratio, ratio);
  ctx.strokeStyle = this.getStroke().getColor();
  ctx.lineWidth = this.getStroke().getWidth(); // Draw grid
...
ol.control.Imageline = function (options) {
  var element = ol.ext.element.create('DIV', {
    className: (options.className || '') + ' ol-imageline' + (options.target ? '' : ' ol-unselectable ol-control') + (options.collapsed && options.collapsible ? 'ol-collapsed' : '') + ('ontouchstart' in window ? ' ol-touch' : '')
  });
  ...
  this._source = options.source; // Initialize
...
ol.control.Imageline.prototype.setMap = function (map) {
  ...
  if (map) {
    this._listener = [map.on('postcompose', this._drawLink.bind(this)), map.on('moveend', function () {
...
ol.control.Imageline.prototype.collapse = function (b) {
  if (b) this.element.classList.add('ol-collapsed');else this.element.classList.remove('ol-collapsed');
...
ol.control.Imageline.prototype._getTitle = function ()
/* f */
{
  return '';
};
...
ol.control.Imageline.prototype.refresh = function () {
  ...
  this._iline = [];
  if (this.getMap()) this.getMap().render(); // Add a new image
    ...
    if (this._getImage(f)) {
      ...
      img.addEventListener('click', function () {
        if (!this._moving) {
          ...
          this._scrolldiv.scrollLeft = img.offsetLeft + ol.ext.element.getStyle(img, 'width') / 2 - ol.ext.element.getStyle(this.element, 'width') / 2;
      ...
      }.bind(this)); // Prevent image dragging
...
ol.control.Imageline.prototype.select = function (feature, scroll) {
  this._select = false; // Find the image
  ...
  this._iline.forEach(function (f) {
    if (f.feature === feature) {
      ...
      if (scroll !== false) {
        this._scrolldiv.scrollLeft = f.elt.offsetLeft + ol.ext.element.getStyle(f.elt, 'width') / 2 - ol.ext.element.getStyle(this.element, 'width') / 2;
      }
    } else {
...
ol.control.Imageline.prototype._drawLink = function (e) {
  ...
  if (map && this._select && this._select.elt) {
    var ctx = e.context;
    var ratio = e.frameState.pixelRatio;
    var pt = [this._select.elt.offsetLeft - this._scrolldiv.scrollLeft + ol.ext.element.getStyle(this._select.elt, 'width') / 2, parseFloat(ol.ext.element.getStyle(this.element, 'top')) || this.getMap().getSize()[1]];
...
ol.control.IsochroneGeoportail = function (options) {
  ...
  this._addSearchCtrl(content, options); // Method buttons
  ...
  this._ajax.on('error', this._error.bind(this)); // searching
...
ol.control.IsochroneGeoportail.prototype.search = function (coord, option, iter) {
  ...
  if (typeof option === 'string') {
    ...
    method = 'time';
    option = parseFloat(option); // convert unit
...
ol.control.IsochroneGeoportail.prototype._success = function (e) {
  var proj = this.getMap() ? this.getMap().getView().getProjection() : 'EPSG:3857'; // Convert to features
...
ol.control.Legend.prototype.getStyleImage = function (options, theCanvas, row) {
  ...
  if (!feature && options.properties && typeGeom) {
    if (/Point/.test(typeGeom)) feature = new ol.Feature(new ol.geom.Point([0, 0]));else if (/LineString/.test(typeGeom)) feature = new ol.Feature(new ol.geom.LineString([0, 0]));else feature = new ol.Feature(new ol.geom.Polygon([[0, 0]]));
...
ol.control.MapZone = function (options) {
  ...
  options.zones.forEach(function (z) {
    ...
    maps.push(map);
    view.fit(extent); // Nmae

    ol.ext.element.create('P', {
      html: z.title,
      parent: div
    });
  }.bind(this)); // Refresh the maps
...
ol.control.MapZone.prototype.setVisible = function (b) {
  if (b) this.element.classList.remove('ol-collapsed');else this.element.classList.add('ol-collapsed');
};
/** Pre-defined zones */
...
ol.control.Overlay.prototype.toggle = function () {
  if (this.getVisible()) this.hide();else this.show();
};
/** Get the control visibility
* @return {boolean} b
*/
...
ol.control.Overview = function (options) {
  ...
  this.oview_ = this.ovmap_.getView(); // Cache extent
  ...
  */
  ...
  var elasticFn = function elasticFn(bounce, amplitude) {
  ...
  this.ovmap_.addInteraction(new ol.interaction.Pointer({
    handleDownEvent: function handleDownEvent(evt) {
...
ol.control.Overview.prototype.setView = function (e) {
  ...
  switch (e.key) {
    case 'rotation':
      {
        if (this.rotation) this.oview_.setRotation(this.getMap().getView().getRotation());else if (this.oview_.getRotation()) this.oview_.setRotation(0);
        break;
      }
    ...
    case 'center':
      {
        ...
        if (mapExtent[0] < extent[0] || mapExtent[1] < extent[1] || mapExtent[2] > extent[2] || mapExtent[3] > extent[3]) {
...
ol.control.Permalink = function (opt_options) {
  ...
  function linkto() {
    if (typeof options.onclick == 'function') options.onclick(self.getLink());else self.setUrlReplace(!self.replaceState_);
  }
  ...
  this.on('change', this.viewChange_.bind(this)); // Save search params
...
ol.control.Permalink.prototype.setMap = function (map) {
  ...
  this._listener = null;
  ol.control.Control.prototype.setMap.call(this, map); // Get change 
...
ol.control.Permalink.prototype.getLayerByLink = function (id, layers) {
  ...
  for (var i = 0; i < layers.length; i++) {
    if (layers[i].get('permalink') == id) return layers[i]; // Layer Group
...
ol.control.Permalink.prototype.setPosition = function () {
  ...
  if (param.r) map.getView().setRotation(Number(param.r)); // Reset layers visibility
  ...
  function resetLayers(layers) {
    ...
    for (var i = 0; i < layers.length; i++) {
      if (layers[i].get('permalink')) {
        layers[i].setVisible(false); // console.log("hide "+layers[i].get('permalink'));
      }
...
ol.control.Permalink.prototype.setUrlParam = function (key, value) {
  if (key) {
    if (value === undefined || value === '') delete this.search_[encodeURIComponent(key)];else this.search_[encodeURIComponent(key)] = encodeURIComponent(value);
  }
...
ol.control.Permalink.prototype.getLink = function (param) {
  ...
  var l = this.layerStr_; // Change anchor
  ...
  if (param) return anchor; //return document.location.origin+document.location.pathname+this.hash_+anchor;
...
ol.control.Print.prototype.print = function (options) {
  ...
  if (this.getMap()) {
    ...
    this.getMap().once('rendercomplete', function (event) {
      var canvas, ctx; // ol <= 5 : get the canvas
      ...
      if (event.context) {
        ...
        this.getMap().getViewport().querySelectorAll('.ol-layer canvas, canvas.ol-fixedoverlay').forEach(function (c) {
          if (c.width) {
            ...
            ctx.globalAlpha = c.style.opacity || 1; // transform
...
ol.control.Profil = function (opt_options) {
  ...
  if (options.target) {
    ...
    var click_touchstart_function = function click_touchstart_function(e) {
      self.toggle();
      e.preventDefault();
    };
  ...
  if (!this.info.xtitle) this.margin_.bottom -= 20 * ratio; // Cursor
  ...
  this.cursor_.appendChild(this.popup_); // Track information
  ...
  secondTr.appendChild(div_time2); // Array of data

  this.tab_ = []; // Show feature
...
ol.control.Profil.prototype.onMove = function (e) {
  ...
  if (dx > this.margin_.left / ratio && dx < (this.canvas_.width - this.margin_.right) / ratio && dy > this.margin_.top / ratio && dy < (this.canvas_.height - this.margin_.bottom) / ratio) {
    ...
    if (dx > this.canvas_.width / ratio / 2) this.popup_.classList.add('ol-left');else this.popup_.classList.remove('ol-left');
    this.dispatchEvent({
      type: 'over',
      click: e.type == "click",
      coord: p[3],
...
ol.control.Profil.prototype.setGeometry = function (g, options) {
  ...
  ctx.clearRect(0, 0, w, h); // No Z

  if (!/Z/.test(g.getLayout())) return; // No time

  if (/M/.test(g.getLayout())) this.element.querySelector(".time").parentElement.style.display = 'block';else this.element.querySelector(".time").parentElement.style.display = 'none'; // Coords
  ...
  h -= this.margin_.top + this.margin_.bottom; // Draw axes
  ...
  for (i = 0, p; p = c[i]; i++) {
    ...
    if (z < zmin) zmin = z;
    if (z > zmax) zmax = z;
    if (i == 0) d = 0;else d += dist2d(c[i - 1], p);
  ...
  this.element.querySelector(".track-info .time").textContent = ti; // Set graduation
  ...
  this.scale_ = [scx, scy]; // Draw

  ctx.font = 10 * ratio + "px arial";
  ctx.textAlign = "right";
  ctx.textBaseline = "middle";
  ctx.fillStyle = "#000"; // Scale Z

  ctx.beginPath();

  for (i = zmin; i <= zmax; i += grad) {
    if (options.zunit != "km") ctx.fillText(i, -4 * ratio, i * scy + dy);else ctx.fillText((i / 1000).toFixed(1), -4 * ratio, i * scy + dy);
    ctx.moveTo(-2 * ratio, i * scy + dy);
    if (i != 0) ctx.lineTo(d * scx, i * scy + dy);else ctx.lineTo(0, i * scy + dy);
  } // Scale X
  ...
  if (d > 1000) {
    ...
    unit = "m";
    if (d > 100) step = Math.round(d / 100) * 10;else if (d > 10) step = Math.round(d / 10);else if (d > 1) step = Math.round(d) / 10;else step = d;
  }

  for (i = 0; i <= d; i += step) {
    var txt = unit == "m" ? i : i / 1000; //if (i+step>d) txt += " "+ (options.zunits || "km");
...
ol.control.RoutingGeoportail.prototype.addSearch = function (element, options, after) {
  var self = this;
  var div = ol.ext.element.create('DIV');
  if (after) element.insertBefore(div, after.nextSibling);else element.appendChild(div);
  ol.ext.element.create('BUTTON', {
...
ol.control.RoutingGeoportail.prototype.listRouting = function (routing) {
  ...
  for (var i = 0, f; f = routing.features[i]; i++) {
    ...
    li.innerHTML = (info[f.get('instruction') || 'none'] || '#') + ' ' + f.get('name') + '<i>' + d + (t ? ' - ' + t : '') + '</i>';
    ul.appendChild(li);
  }
};
...
ol.control.RoutingGeoportail.prototype.handleResponse = function (data, start, end) {
  ...
  */
  ...
  routing.duration = parseFloat(data.durationSeconds); // Full route
...
ol.control.RoutingGeoportail.prototype.ajax = function (url, onsuccess, onerror) {
  var self = this; // Abort previous request
  ...
  this.element.classList.add('ol-searching'); // Load complete
...
ol.control.Scale.prototype._showScale = function () {
  ...
  if (map) {
    ...
    var d = ol.sphere.getDistance(ol.proj.transform(center, proj, 'EPSG:4326'), ol.proj.transform(coord, proj, 'EPSG:4326'));
    d *= this.get('ppi') / .0254;
...
ol.control.Scale.prototype.formatScale = function (d) {
  if (d > 100) d = Math.round(d / 100) * 100;else d = Math.round(d);
...
ol.control.SearchDFCI.prototype.autocomplete = function (s) {
  ...
  dfci = dfci.replace(/[^0-9,^A-H,^K-N]/g, ''); // Valid DFCI ?
...
ol.control.SearchFeature.prototype.autocomplete = function (s) {
  ...
  if (this.source_) {
    // regexp
    s = s.replace(/^\*/, '');
    var rex = new RegExp(s, 'i'); // The source
...
ol.control.SearchGPS = function (options) {
  ...
  ol.control.Search.call(this, options); // Geolocation
  ...
  ol.ext.element.create('INPUT', {
    ...
    on: {
      'change': function (e) {
        if (e.target.checked) this.element.classList.add('ol-dms');else this.element.classList.remove('ol-dms');
      }.bind(this)
    }
  });
  ol.ext.element.create('SPAN', {
  ...
  this._createForm(); // Move list to the end
...
ol.control.SearchGPS.prototype._createForm = function () {
  ...
  var lons = createInput('ol-dms', '"'); // Latitude
  ...
  var lats = createInput('ol-dms', '"'); // Focus
  ...
  }.bind(this)); // Change value on geolocation
...
ol.control.SearchGeoportailParcelle = function (options) {
  ...
  this.set('copy', null);
  var element = this.element; // Add parcel form
  ...
  div.appendChild(label);
  div.appendChild(document.createElement("BR")); // Input
  ...
  this._inputParcelle.numero.setAttribute('maxlength', 4); // Delay search
  ...
  this.activateParcelle(false); // Autocomplete list
  ...
  auto.appendChild(ul); // Show/hide list on fcus/blur	
...
ol.control.SearchGeoportailParcelle.prototype.autocompleteParcelle = function () {
  ...
  this.searchParcelle(search, function (jsonResp) {
...
ol.control.SearchGeoportailParcelle.prototype.searchParcelle = function (search, success
/*, error */
) {
  // Request
  var request = '<?xml version="1.0" encoding="UTF-8"?>' + '<XLS xmlns:xls="http://www.opengis.net/xls" xmlns:gml="http://www.opengis.net/gml" xmlns="http://www.opengis.net/xls" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.2" xsi:schemaLocation="http://www.opengis.net/xls http://schemas.opengis.net/ols/1.2/olsAll.xsd">' + '<RequestHeader/>' + '<Request requestID="1" version="1.2" methodName="LocationUtilityService">' + '<GeocodeRequest returnFreeForm="false">' + '<Address countryCode="CadastralParcel">' + '<freeFormAddress>' + search + '+</freeFormAddress>' + '</Address>' + '</GeocodeRequest>' + '</Request>' + '</XLS>'; // Geocode
...
ol.control.SearchNominatim.prototype.select = function (f) {
  var c = [Number(f.lon), Number(f.lat)]; // Add coordinate to the event
...
ol.control.SearchNominatim.prototype.reverseGeocode = function (coord, cback) {
  var lonlat = ol.proj.transform(coord, this.getMap().getView().getProjection(), 'EPSG:4326');
  this.ajax(this.get('url').replace('search', 'reverse'), {
    lon: lonlat[0],
    lat: lonlat[1],
    format: 'json'
  }, function (resp) {
    if (cback) {
      ...
      this._handleSelect(resp, true); //this.setInput('', true);
...
ol.control.SearchWikipedia.prototype.select = function (f) {
  var title = decodeURIComponent(f.uri.split('/').pop()).replace(/'/, '%27'); // Search for coords
...
ol.control.Select = function (options) {
  ...
  var div = options.content = document.createElement("div"); // Autocompletion list
  ...
  ol.ext.element.appendText(label_match_all, options.allLabel || 'match all'); // Use case
  ...
  ol.control.SelectBase.call(this, options); // Add button
...
ol.control.Select.prototype.getConditionsString = function (cond) {
  ...
  for (var i = 0, c; c = cond.conditions[i]; i++) {
    if (c.attr) {
      st += (st ? cond.all ? ' AND ' : ' OR ' : '') + c.attr + this.operationsList[c.op] + c.val;
    }
  }
...
ol.control.Select.prototype._getLiCondition = function (i) {
  var self = this;
  var li = document.createElement('li'); // Attribut
  ...
  autocomplete.appendChild(input_attr); // Autocomplete list

  var ul_autocomplete = document.createElement('ul');
  ul_autocomplete.classList.add('ol-hidden');
  autocomplete.appendChild(ul_autocomplete); // Operation
...
ol.control.SelectCheck = function (options) {
  ...
  this._onchoice = options.onchoice; // Set select options
...
ol.control.SelectCheck.prototype.setValues = function (options) {
  ...
  var addCheck = function (val, info) {
    ...
    this._checks.push(ol.ext.element.create('INPUT', {
      ...
      value: val,
      change: function () {
        if (this._onchoice) this._onchoice();else this.doSelect();
      }.bind(this),
      parent: label
    }));
...
ol.control.SelectCondition.prototype.setCondition = function (condition) {
  if (!condition) this._conditions = [];else this._conditions = condition instanceof Array ? condition : [condition];
};
...
ol.control.Status = function (options) {
  ...
  var element = ol.ext.element.create('DIV', {
    className: (options.className || '') + ' ol-status' + (options.target ? '' : ' ol-unselectable ol-control')
  }); // Initialize
...
ol.control.Status.prototype.status = function (html) {
  ...
  if (s) {
    ...
    if (_typeof(s) === 'object' && !(s instanceof String)) {
      s = '';
...
ol.control.Status.prototype.show = function (show) {
  if (show === false) ol.ext.element.hide(this.element);else ol.ext.element.show(this.element);
};
/** Hide the status
 */
...
ol.control.Storymap = function (options) {
  ...
  var element = ol.ext.element.create('DIV', {
    className: (options.className || '') + ' ol-storymap' + (options.target ? '' : ' ol-unselectable ol-control') + ('ontouchstart' in window ? ' ol-touch' : ''),
    html: options.html
  });
  ...
  }.bind(this)); // Trigger change event on scroll

  this.element.addEventListener("scroll", function () {
    var current,
        chapter = this.element.querySelectorAll('.chapter');
...
ol.control.Swipe.prototype.setMap = function (map) {
  ...
  if (this.getMap()) {
    for (i = 0; i < this.layers.length; i++) {
      l = this.layers[i];
      if (l.right) l.layer.un(['precompose', 'prerender'], this.precomposeRight_);else l.layer.un(['precompose', 'prerender'], this.precomposeLeft_);
  ...
  if (map) {
    ...
    for (i = 0; i < this.layers.length; i++) {
      l = this.layers[i];
      if (l.right) l.layer.on(['precompose', 'prerender'], this.precomposeRight_);else l.layer.on(['precompose', 'prerender'], this.precomposeLeft_);
...
ol.control.Swipe.prototype.addLayer = function (layers, right) {
  ...
  for (var i = 0; i < layers.length; i++) {
    ...
    if (this.isLayer_(l) < 0) {
      ...
      if (this.getMap()) {
        if (right) l.on(['precompose', 'prerender'], this.precomposeRight_);else l.on(['precompose', 'prerender'], this.precomposeLeft_);
...
ol.control.Swipe.prototype.removeLayer = function (layers) {
  ...
  for (var i = 0; i < layers.length; i++) {
    ...
    if (k >= 0 && this.getMap()) {
      if (this.layers[k].right) layers[i].un(['precompose', 'prerender'], this.precomposeRight_);else layers[i].un(['precompose', 'prerender'], this.precomposeLeft_);
...
ol.control.Swipe.prototype.precomposeLeft = function (e) {
  ...
  ctx.save();
  ctx.beginPath();
  if (this.get('orientation') === "vertical") ctx.rect(0, 0, canvas.width * this.get('position'), canvas.height);else ctx.rect(0, 0, canvas.width, canvas.height * this.get('position'));
  ctx.clip();
};
/** @private
*/
...
ol.control.Swipe.prototype.precomposeRight = function (e) {
  ...
  ctx.save();
  ctx.beginPath();
  if (this.get('orientation') === "vertical") ctx.rect(canvas.width * this.get('position'), 0, canvas.width, canvas.height);else ctx.rect(0, canvas.height * this.get('position'), canvas.width, canvas.height);
  ctx.clip();
};
/** @private
*/
...
ol.control.Target = function (options) {
  options = options || {};
  this.style = options.style || [new ol.style.Style({
...
ol.control.Timeline = function (options) {
  var element = ol.ext.element.create('DIV', {
    className: (options.className || '') + ' ol-timeline' + (options.target ? '' : ' ol-unselectable ol-control') + (options.zoomButton ? ' ol-hasbutton' : '') + ('ontouchstart' in window ? ' ol-touch' : '')
  }); // Initialize
  ...
  }.bind(this)); // Trigger scroll event
  ...
  }.bind(this)); // Magic to give "live" scroll events on touch devices


  this._scrollDiv.addEventListener('gesturechange', function () {}); // Scroll timeline
  ...
  if (options.endFeatureDate) this._endFeatureDate = options.endFeatureDate; // Feature source 
...
ol.control.Timeline.prototype.setInterval = function (length) {
  ...
  if (length) this.element.classList.add('ol-interval');else this.element.classList.remove('ol-interval');
...
ol.control.Timeline.prototype._endFeatureDate = function ()
/* feature */
{
  return undefined;
};
...
ol.control.Timeline.prototype.setFeatures = function (features, zoom) {
  ...
  if (features instanceof ol.source.Vector) this._source = features;else if (features instanceof Array) this._features = features;else this._features = [];
  this.refresh(zoom);
};
/**
 * Get features
...
ol.control.Timeline.prototype.refresh = function (zoom, first) {
  ...
  this._scrollDiv.innerHTML = '';
  var features = this.getFeatures();
  var d, d2; // Get features sorted by date
  ...
  var scale = this._scale = (delta > maxWidth ? maxWidth / delta : 1) * zoom; // Leave 10px on right
  ...
  this._drawTime(div, min, max, scale); // Set interval
  ...
  var line = [];
  var lineHeight = ol.ext.element.getStyle(this._scrollDiv, 'lineHeight'); // Wrapper
  ...
  tline.forEach(function (f) {
    ...
    var left = ol.ext.element.getStyle(t, 'left'); // Select on click
    ...
    }.bind(this)); // Find first free Y position
...
ol.control.Timeline.prototype._drawTime = function (div, min, max, scale) {
  ...
  var heigth = ol.ext.element.getStyle(tdiv, 'height'); // Year
...
ol.featureAnimation.prototype.drawGeom_ = function (e, geom, shadow) {
  ...
  for (var i = 0; i < style.length; i++) {
    var sc = 0; // OL < v4.3 : setImageStyle doesn't check retina
...
ol.featureAnimation.prototype.animate = function ()
/* e */
{
  return false;
};
...
ol.layer.Base.prototype.animateFeature = function (feature, fanim, useFilter) {
  ...
  function start(options) {
    if (fanim.length && !listenerKey) {
      ...
      feature.setStyle(fanim[step].hiddenStyle || new ol.style.Style({
        image: new ol.style.Circle({})
      })); // Send event
...
ol.featureAnimation.Path.prototype.animate = function (e) {
  ...
  var p0,
      ...
      dl,
      d = 0;
  p = this.path_[0]; // Linear interpol
  ...
  e.geom.setCoordinates(p); // Animate
...
ol.featureAnimation.Shake = function (options) {
  options = options || {};
  ol.featureAnimation.call(this, options); //	this.easing_ = options.easing_ || function(t){return (0.5+t)*t -0.5*t ;};
...
ol.featureAnimation.Slide.prototype.animate = function (e) {
  // First time > calculate duration / speed
  if (!e.time) {
    if (this.side_ == 'left') this.dx = e.extent[0] - e.bbox[2];else this.dx = e.extent[2] - e.bbox[0];
...
ol.featureAnimation.Throw.prototype.animate = function (e) {
  ...
  var flashGeom = e.geom.clone();
  var shadow = e.geom.clone();
  flashGeom.translate(this.dx * (1 - this.easing_(e.elapsed)), this.dy * Math.cos(Math.PI / 2 * this.easing_(e.elapsed)));
...
ol.filter.Base = function (options) {
  ol.Object.call(this); // Array of postcompose listener

  this._listener = [];
  if (options && options.active === false) this.set('active', false);else this.set('active', true);
};
...
(function () {
  ...
  */
  ...
  function filterRedraw_() {
    if (this.renderSync) this.renderSync();else this.changed();
  }
...
ol.filter.Mask.prototype.drawFeaturePath_ = function (e, out) {
  var ctx = e.context;
  var canvas = ctx.canvas;
  var ratio = e.frameState.pixelRatio; // Transform

  var m = e.frameState.coordinateToPixelTransform;

  var tr = function tr(pt) {
    return [(pt[0] * m[0] + pt[1] * m[1] + m[4]) * ratio, (pt[0] * m[2] + pt[1] * m[3] + m[5]) * ratio];
  }; // Old ol version
  ...
  if (!m) {
    ...
    tr = function tr(pt) {
      return [(pt[0] * m[0] + pt[1] * m[1] + m[12]) * ratio, (pt[0] * m[4] + pt[1] * m[5] + m[13]) * ratio];
    };
  } // Geometry
...
ol.filter.Colorize.prototype.setFilter = function (options) {
  ...
  switch (options.operation) {
    ...
    case 'luminosity':
      v = 255 * (options.value || 0);
      this.set('color', ol.color.asString([v, v, v, 255])); //this.set ('operation', 'luminosity')
...
ol.filter.Lego = function (options) {
  if (!options) options = {};
  ol.filter.Base.call(this, options);
  var img = new Image(); // Default image

  img.src = this.img[options.img] || this.img.ol3;
  img.crossOrigin = options.crossOrigin || null; // and pattern 
...
ol.filter.Lego.prototype.getPattern = function (offsetX, offsetY) {
  if (!this.pattern.ctx) return "transparent"; //return this.pattern.ctx.fillStyle
...
ol.filter.Lego.prototype.postcompose = function (e) {
  ...
  var h = this.internal_.height = canvas.height; // No smoothing please

  ctx2.webkitImageSmoothingEnabled = ctx2.mozImageSmoothingEnabled = ctx2.msImageSmoothingEnabled = ctx2.imageSmoothingEnabled = false;
  ...
  ctx.webkitImageSmoothingEnabled = ctx.mozImageSmoothingEnabled = ctx.msImageSmoothingEnabled = ctx.imageSmoothingEnabled = false;
...
ol.filter.Texture.prototype.setFilter = function (options) {
  var img;
  options = options || {};
  if (options.img) img = options.img;else {
    img = new Image();
  ...
  function setPattern(img) {
    ...
    self.pattern.ctx.fillStyle = self.pattern.ctx.createPattern(img, 'repeat'); // Force refresh
...
ol.filter.Texture.prototype.postcompose = function (e) {
  // not ready
  if (!this.pattern) return; // Set back color hue

  var ctx = e.context;
  var canvas = ctx.canvas;
  var m = 1.5 * Math.max(canvas.width, canvas.height);
  var mt = e.frameState.pixelToCoordinateTransform; // Old version (matrix)

  if (!mt) {
    mt = e.frameState.pixelToCoordinateMatrix, mt[2] = mt[4];
    mt[3] = mt[5];
    mt[4] = mt[12];
    mt[5] = mt[13];
  }
  ...
  var w = canvas.width / 2,
      h = canvas.height / 2;
  ctx.save();
  ctx.globalCompositeOperation = "multiply"; //ctx.globalCompositeOperation = "overlay";
  ...
  if (this.get('rotateWithView')) {
    ...
    ctx.fillStyle = this.getPattern((w * mt[0] + h * mt[1] + mt[4]) / res, (w * mt[2] + h * mt[3] + mt[5]) / res); // Rotate on canvas center and fill
...
ol.interaction.CenterTouch = function (options) {
  options = options || {}; // LIst of listerner on the object

  this._listener = {}; // Filter event

  var rex = /^pointermove$|^pointerup$/; // Default style = cross
  ...
  this.composite = options.composite || ''; // Interaction to defer center on top of the interaction 
...
ol.interaction.Clip = function (options) {
  ...
  this.postcomposeBind_ = this.postcompose_.bind(this); // Default options
...
ol.interaction.Clip.prototype.setPosition = function (e) {
  if (e.pixel) this.pos = e.pixel;else {
    if (e && e instanceof Array) this.pos = e;else e = [-10000000, -10000000];
  }
  if (this.getMap()) this.getMap().renderSync();
};
/* @private
...
ol.interaction.CopyPaste = function (options) {
  options = options || {}; // Features to copy
  ...
  this.setDestination(options.destination); // Create intreaction
...
ol.interaction.CopyPaste.prototype.copy = function (options) {
  ...
  var features = options.features || this._featuresSource.getArray(); // Try to remove feature from sources


  if (options.cut) {
    var sources = this._source || [this._destination]; // Remove feature from sources
...
ol.interaction.Delete.prototype.delete = function (features) {
  if (features && (features.length || features.getLength())) {
    ...
    var delFeatures = []; // Get the sources concerned
...
ol.interaction.DragOverlay = function (options) {
  if (!options) options = {}; // Extend pointer
  ...
  ol.interaction.Pointer.call(this, {
    ...
    // Drag
    handleDragEvent: function handleDragEvent(evt) {
      if (this._dragging) {
        this._dragging.setPosition(evt.coordinate);
    ...
    // Stop dragging
    handleUpEvent: function handleUpEvent(evt) {
      if (this._dragging) {
        this.dispatchEvent({
          type: 'dragend',
          overlay: this._dragging,
...
ol.interaction.DrawHole = function (options) {
  if (!options) options = {};
  var self = this; // Select interaction for the current feature

  this._select = new ol.interaction.Select({
    style: options.style
  });

  this._select.setActive(false); // Geometry function that test points inside the current


  var geometryFn,
      geomFn = options.geometryFunction;

  if (geomFn) {
    geometryFn = function geometryFn(c, g) {
  ...
  ol.interaction.Draw.call(this, options); // Layer filter function
  ...
  this.on('drawstart', this._startDrawing.bind(this)); // End drawing add the hole to the current Polygon
...
ol.interaction.DrawHole.prototype.getPolygon = function () {
  return this._polygon; // return this._select.getFeatures().item(0).getGeometry();
};
/**
...
ol.interaction.DrawHole.prototype._startDrawing = function (e) {
  ...
  var coord = e.feature.getGeometry().getCoordinates()[0][0]; // Check object under the pointer

  var features = map.getFeaturesAtPixel(map.getPixelFromCoordinate(coord), {
...
ol.interaction.DrawHole.prototype._finishDrawing = function (e) {
  ...
  e.hole = e.feature; // Get the current feature
...
ol.interaction.DrawRegular = function (options) {
  ...
  this.maxCircleCoordinates_ = options.maxCircleCoordinates || 100; // Collection of feature to transform 

  this.features_ = options.features; // List of layers to transform 

  this.source_ = options.source; // Square condition

  this.squareFn_ = options.squareCondition; // Centered condition

  this.centeredFn_ = options.centerCondition; // Allow rotation when centered + square

  this.canRotate_ = options.canRotate !== false; // Specify custom geometry name

  this.geometryName_ = options.geometryName; // Number of sides (default=0: circle)

  this.setSides(options.sides); // Style
...
ol.interaction.DrawRegular.prototype.getGeom_ = function () {
  ...
  if (this.coord_) {
    ...
    var coord = this.coord_; // Specific case: circle
    ...
    if (!this.sides_ && this.square_ && !this.centered_) {
      ...
      circle = new ol.geom.Circle(center, r, 'XY'); // Optimize points on the circle
      ...
      if (this.square_ && !hasrotation) {
        ...
        coord = [center[0] + (d[0] > 0 ? dm : -dm), center[1] + (d[1] > 0 ? dm : -dm)];
      }
      ...
      if (r > 0) {
        ...
        var a;
        if (hasrotation) a = Math.atan2(d[1], d[0]);else a = this.startAngle[this.sides_] || this.startAngle['default'];
        ...
        if (hasrotation) return g; // Scale polygon to fit extent

        var ext = g.getExtent();
        if (!this.centered_) center = this.center_;else center = [2 * this.center_[0] - this.coord_[0], 2 * this.center_[1] - this.coord_[1]];
...
ol.interaction.DrawRegular.prototype.drawSketch_ = function (evt) {
  ...
  if (evt) {
    ...
    if (g) {
      var f = this.feature_;
      if (this.geometryName_) f.setGeometryName(this.geometryName_); //f.setGeometry (g);
...
ol.interaction.DrawRegular.prototype.handleEvent_ = function (evt) {
  ...
  switch (evt.type) {
    case "pointerdown":
      {
        ...
        this.start_(evt); // Test long touch
    ...
    default:
      {
        this.lastEvent = evt.type; // Prevent zoom in on dblclick
...
ol.interaction.DrawRegular.prototype.end_ = function (evt) {
  this.coord_ = evt.coordinate;
  this.started_ = false; // Add new feature

  if (this.coord_ && this.center_[0] != this.coord_[0] && this.center_[1] != this.coord_[1]) {
    var f = this.feature_;
    if (this.geometryName_) f.setGeometryName(this.geometryName_);
    f.setGeometry(this.getGeom_());
    if (this.source_) this.source_.addFeature(f);else if (this.features_) this.features_.push(f);
    this.dispatchEvent({
      type: 'drawend',
      feature: f,
      pixel: evt.pixel,
...
ol.interaction.DrawTouch = function (options) {
  ...
  this.source_ = options.source;
  this.set("tap", options.tap !== false); // Style
...
ol.interaction.DrawTouch.prototype.finishDrawing = function () {
  ...
  if (f) this.source_.addFeature(f); // reset
...
ol.interaction.DrawTouch.prototype.drawSketchLink_ = function (e) {
  ...
  ctx.save();
  var p,
      pt = this.getMap().getPixelFromCoordinate(this.getPosition());
...
ol.interaction.DropFile = function (options) {
  ...
  zone.addEventListener('dragleave', this.onstop); // Options
...
ol.interaction.DropFile.prototype.ondrop = function (e) {
  ...
  if (e.dataTransfer && e.dataTransfer.files.length) {
    var self = this; // fetch FileList object
    ...
    for (var i = 0; file = files[i]; i++) {
      ...
      var tryReadFeatures = function tryReadFeatures(format, result, options) {
        try {
...
ol.interaction.FillAttribute.prototype.setActive = function (active) {
  ...
  if (this.getMap() && this._cursor) {
    if (active) {
      ...
      this.getMap().getTargetElement().style.cursor = this._cursor; //      console.log('setCursor',this._cursor)
    } else {
...
ol.interaction.Flashlight.prototype.setColor = function (options) {
  ...
  this.startColor = ol.color.asString(c); // Halo color
...
ol.interaction.Flashlight.prototype.setPosition = function (e) {
  if (e.pixel) this.pos = e.pixel;else this.pos = e;
...
ol.interaction.Flashlight.prototype.postcompose_ = function (e) {
  ...
  if (!this.pos) {
    ...
    var d = Math.max(w, h); // reveal wherever we drag
...
ol.interaction.FocusMap = function () {
  //
  ol.interaction.Interaction.call(this, {}); // Focus (hidden) button to focus on the map when click on it 
...
ol.interaction.GeolocationDraw = function (options) {
  ...
  this.geolocation.on('change', this.draw_.bind(this)); // Current path

  this.path_ = [];
  this.lastPosition_ = false; // Default style
  ...
  var defaultStyle = function defaultStyle(f) {
...
ol.interaction.GeolocationDraw.prototype.setFollowTrack = function (follow) {
  ...
  var map = this.getMap(); // Center if wanted
...
ol.interaction.GeolocationDraw.prototype.draw_ = function () {
  ...
  var p = loc.getAccuracyGeometry(); // Center on point
    ...
    case 'auto':
      {
        if (this.lastPosition_) {
          var center = map.getView().getCenter(); // console.log(center,this.lastPosition_)
  ...
  f.setGeometry(p);
  if (accu < this.get("minAccuracy") / 2) f.setStyle(this.locStyle.ok);else if (accu < this.get("minAccuracy")) f.setStyle(this.locStyle.warn);else f.setStyle(this.locStyle.error);
  var geo;
  ...
  this.sketch_[2].set("heading", loc.getHeading()); // Drawing
...
ol.interaction.ModifyFeature = function (options) {
    ...
    */
    handleEvent: function handleEvent(e) {
      switch (e.type) {
        ...
        case 'pointerdrag':
          {
            if (dragging) return this.handleDragEvent(e);else return true;
          }

        case 'pointermove':
          {
            if (!dragging) return this.handleMoveEvent(e);else return true;
          }
  ...
  this.snapDistance_ = options.pixelTolerance || 10; // Split tolerance between the calculated intersection and the geometry

  this.tolerance_ = 1e-10; // Cursor

  this.cursor_ = options.cursor; // List of source to split
  ...
  this._insertVertexCondition = options.insertVertexCondition || ol.events.condition.always; // Default style

  var sketchStyle = function sketchStyle() {
  ...
  if (options.style) {
    if (typeof options.style === 'function') {
      ...
      sketchStyle = function sketchStyle() {
    ...
    case 'LineString':
      {
        ...
        if (ol.coordinate.dist2d(p, coord) < 1.5 * this.tolerance_) {
          var split; // Split the line in two
          ...
          if (split.length === 2) {
            ...
            var end = ol.coordinate.equal(s[s.length - 1], coord); // Move first point
...
ol.interaction.ModifyFeature.prototype.handleDownEvent = function (evt) {
  if (!this.getActive()) return false; // Something to split ?

  var current = this.getClosestFeature(evt);

  if (current && (this._condition(evt) || this._deleteCondition(evt))) {
    var features = [];
    this.arcs = []; // Get features concerned
    ...
    case 'GeometryCollection':
      {
        ...
        var found = this._getModification(a); // Restore current arc
...
ol.interaction.ModifyFeature.prototype._removePoint = function (current, evt) {
  ...
  this.overlayLayer_.getSource().clear();
  var found = false; // Get all modifications

  this.arcs.forEach(function (a) {
    found = found || this._getModification(a);
  }.bind(this)); // Almost one point is removed
...
ol.interaction.ModifyFeature.prototype.handleDragEvent = function (e) {
  if (!this.getActive()) return false;
  if (!this.arcs) return true; // Show sketch

  this.overlayLayer_.getSource().clear();
  var p = new ol.Feature(new ol.geom.Point(e.coordinate));
  this.overlayLayer_.getSource().addFeature(p); // Nothing to do

  if (!this.arcs.length) return true; // Move arcs
...
ol.interaction.ModifyFeature.prototype.handleMoveEvent = function (e) {
  if (!this.getActive()) return false;
  this.overlayLayer_.getSource().clear();
  var current = this.getClosestFeature(e); // Draw sketch
...
ol.interaction.ModifyTouch = function (options) {
  ...
  this._features = options.features; // popup content
  ...
  var searchDist = pixelTolerance + 5; // Check if there is a feature to select
...
ol.interaction.ModifyTouch.prototype.removePoint = function () {
  ...
  if (new Date() - this._timeout < 200) return; // Remove point
...
ol.interaction.ModifyTouch.prototype.showDeleteBt = function (e) {
  ...
  e.type += 'popup';
  this.dispatchEvent(e); // Date if popup start a timeout to prevent touch + click on the popup
...
ol.interaction.Offset = function (options) {
  ...
  this.layers_ = options.layers ? options.layers instanceof Array ? options.layers : [options.layers] : null; // duplicate

  this.set('duplicate', options.duplicate);
  this.source_ = options.source; // init
...
ol.interaction.Offset.prototype.getFeatureAtPixel_ = function (e) {
  var self = this;
  return this.getMap().forEachFeatureAtPixel(e.pixel, function (feature, layer) {
    var current; // feature belong to a layer
    ...
    if (current && /Polygon|LineString/.test(typeGeom)) {
      if (typeGeom === 'Polygon' && current.getGeometry().getCoordinates().length > 1) return false; // test distance
...
ol.interaction.Ripple = function (options) {
  ...
  this.ripplemap = [];
  this.last_map = []; // Generate random ripples
...
ol.interaction.Ripple.prototype.postcompose_ = function (e) {
  var ctx = e.context;
  var canvas = ctx.canvas; // Initialize when canvas is ready / modified
  ...
  this.ripple = ctx.getImageData(0, 0, this.width, this.height); // Run animation
  ...
  for (var y = 0; y < this.height; y++) {
    for (var x = 0; x < this.width; x++) {
      ...
      data = this.ripplemap[_mapind - this.width] + this.ripplemap[_mapind + this.width] + this.ripplemap[_mapind - 1] + this.ripplemap[_mapind + 1] >> 1;
      data -= this.ripplemap[_newind];
      data -= data >> 5;
      this.ripplemap[_newind] = data; //where data=0 then still, where data>0 then wave

      data = 1024 - data;

      if (this.last_map[i] != data) {
        this.last_map[i] = data; //offsets

        a = ((x - this.half_width) * data / 1024 << 0) + this.half_width;
        b = ((y - this.half_height) * data / 1024 << 0) + this.half_height; //bounds check
  ...
  ctx.putImageData(this.ripple, 0, 0); // tell OL3 to continue postcompose animation
...
ol.interaction.SelectCluster = function (options) {
  ...
  this.selectCluster_ = options.selectCluster !== false; // Create a new overlay layer for 
...
ol.interaction.SelectCluster.prototype.setMap = function (map) {
  ...
  this.overlayLayer_.setMap(map); // map.addLayer(this.overlayLayer_);
...
ol.interaction.SelectCluster.prototype.selectCluster = function (e) {
  ...
  var feature = e.selected[0]; // It's one of ours

  if (feature.get('selectclusterfeature')) return; // Clic out of the cluster => close it

  var source = this.overlayLayer_.getSource();
  source.clear();
  var cluster = feature.get('features'); // Not a cluster (or just one feature)

  if (!cluster || cluster.length == 1) return; // Remove cluster from selection

  if (!this.selectCluster_) this.getFeatures().clear();
  var center = feature.getGeometry().getCoordinates(); // Pixel size in map unit

  var pix = this.getMap().getView().getResolution();
  var r = pix * this.pointRadius * (0.5 + cluster.length / 4);
  var a, i, max;
  var p, cf, lk; // The features

  var features = []; // Draw on a circle
  ...
  if (!this.spiral || cluster.length <= this.circleMaxObjects) {
    ...
    for (i = 0; i < max; i++) {
      ...
      max = Math.min(this.maxObjects, cluster.length); // Feature on a spiral

      for (i = 0; i < max; i++) {
        // New radius => increase d in one turn
        r = d / 2 + d * a / (2 * Math.PI); // Angle
...
ol.interaction.SelectCluster.prototype.animateCluster_ = function (center, features) {
  ...
  function animate(event) {
    var vectorContext = event.vectorContext || ol.render.getVectorContext(event); // Retina device
    ...
    for (var i = 0, feature; feature = features[i]; i++) {
      if (feature.get('features')) {
        ...
        var geo = new ol.geom.Point(pt); // Image style

        var st = stylefn(feature, res);

        for (var s = 0; s < st.length; s++) {
          var sc; // OL < v4.3 : setImageStyle doesn't check retina
  ...
  this.listenerKey_ = this.overlayLayer_.on(['postcompose', 'postrender'], animate.bind(this)); // Start animation with a ghost feature
...
ol.interaction.SnapGuides = function (options) {
  if (!options) options = {}; // Intersect 2 guides
  ...
  this.enableInitialGuides_ = options.enableInitialGuides || false; // Default style
  ...
  if (options.style) sketchStyle = options.style instanceof Array ? options.style : [options.style]; // Create a new overlay for the sketch
...
ol.interaction.SnapGuides.prototype.addGuide = function (v, ortho) {
  if (v) {
    var map = this.getMap(); // Limit extent

    var extent = map.getView().calculateExtent(map.getSize());
    var guideLength = Math.max(this.projExtent_[2] - this.projExtent_[0], this.projExtent_[3] - this.projExtent_[1]);
    extent = ol.extent.buffer(extent, guideLength * 1.5); //extent = ol.extent.boundingExtent(extent, this.projExtent_);
    ...
    var generateLine = function generateLine(loopDir) {
      var p,
          g = [];
      var loopCond = guideLength * loopDir * 2;

      for (var i = 0; loopDir > 0 ? i < loopCond : i > loopCond; i += guideLength * loopDir / 100) {
        if (ortho) p = [v[0][0] + dy * d * i, v[0][1] - dx * d * i];else p = [v[0][0] + dx * d * i, v[0][1] + dy * d * i];
        if (ol.extent.containsCoordinate(extent, p)) g.push(p);else break;
      }
...
ol.interaction.SnapGuides.prototype.setDrawInteraction = function (drawi) {
  var self = this; // Number of points currently drawing

  var nb = 0; // Current guidelines
...
ol.interaction.SnapGuides.prototype.setModifyInteraction = function (modifyi) {
  ...
  var self = this; // Current guidelines
  ...
  modifyi.on("modifystart", setGuides); // end drawing, clear directions
...
ol.interaction.Split = function (options) {
  ...
  this.snapDistance_ = options.snapDistance || 25; // Split tolerance between the calculated intersection and the geometry

  this.tolerance_ = options.tolerance || 1e-10; // Cursor
  ...
  if (options.featureStyle) featureStyle = options.featureStyle instanceof Array ? options.featureStyle : [options.featureStyle]; // Create a new overlay for the sketch
  ...
  this.overlayLayer_ = new ol.layer.Vector({
    ...
    style: function style(f) {
      if (f._sketch_) return sketchStyle;else return featureStyle;
    }
  });
};
...
ol.interaction.Split.prototype.handleMoveEvent = function (e) {
  ...
  if (current && this.filterSplit_(current.feature)) {
    ...
    l._sketch_ = true;
    this.overlayLayer_.getSource().addFeature(l); // move event
...
ol.interaction.Splitter = function (options) {
  ...
  this.tolerance_ = options.tolerance || 1e-10; // Get all features candidate
...
ol.interaction.Splitter.prototype.intersectSegs = function (s1, s2) {
  ...
  if (Math.abs(det) < tol) {
    ...
    if (r2 < 0 || r2 > 1) return false; // Intersection

    var a = s1[0][0] * s1[1][1] - s1[0][1] * s1[1][0];
    var b = s2[0][0] * s2[1][1] - s2[0][1] * s2[1][0];
    var p = [(a * x34 - b * x12) / det, (a * y34 - b * y12) / det]; // Test start / end
...
ol.interaction.Splitter.prototype.splitSource = function (feature) {
  ...
  for (i = 0; i < c.length - 1; i++) {
    seg = [c[i], c[i + 1]];
    var extent = ol.extent.buffer(ol.extent.boundingExtent(seg), this.tolerance_
    /*0.01*/
    );
    var g;
    ...
    while (true) {
      var found = false;
      this.source_.forEachFeatureIntersectingExtent(extent, intersect.bind(this)); // Split feature
    ...
    for (var i = 0; i < this.maps.length; i++) {
      switch (e.type) {
        case 'change:rotation':
          if (this.maps[i].getView().getRotation() != map.getView().getRotation()) this.maps[i].getView().setRotation(map.getView().getRotation());
          break;

        case 'change:center':
          if (this.maps[i].getView().getCenter() != map.getView().getCenter()) this.maps[i].getView().setCenter(map.getView().getCenter());
          break;
...
ol.interaction.Synchronize.prototype.handleMouseOut_ = function ()
/*e*/
{
  for (var i = 0; i < this.maps.length; i++) {
...
ol.Map.prototype.showTarget = function (coord) {
  if (!this.targetOverlay_) {
    ...
    elt.parentElement.classList.add("ol-target-overlay"); // hack to render targetOverlay before positioning it
...
ol.interaction.TinkerBell.prototype.postcompose_ = function (e) {
  ...
  ctx.restore(); // tell OL3 to continue postcompose animation
...
ol.interaction.Transform = function (options) {
  if (!options) options = {};
  var self = this;
  this.selection_ = new ol.Collection(); // Create a new overlay layer for the sketch
  ...
  this.features_ = options.features; // Filter or list of layers to transform
  ...
  this.set('hitTolerance', options.hitTolerance || 0); // Force redraw when changed
...
ol.interaction.Transform.prototype.drawSketch_ = function (center) {
  ...
  var ext = this.selection_.item(0).getGeometry().getExtent(); // Clone and extend
  ...
  if (center === true) {
    ...
    if (this.ispt_) {
      ...
      ext = ol.extent.boundingExtent([this.getMap().getCoordinateFromPixel([p[0] - 10, p[1] - 10]), this.getMap().getCoordinateFromPixel([p[0] + 10, p[1] + 10])]);
    }
...
ol.interaction.Transform.prototype.select = function (feature, add) {
  ...
  if (!feature.getGeometry || !feature.getGeometry()) return; // Add to selection
  ...
  this.drawSketch_();
  this.watchFeatures_(); // select event
...
ol.interaction.Transform.prototype.handleDownEvent_ = function (evt) {
  ...
  if (sel.handle) {
    ...
    this.constraint_ = sel.constraint; // Save info
    ...
    if (this.mode_ === 'rotate') {
      this.center_ = this.getCenter() || ol.extent.getCenter(extent); // we are now rotating (cursor down on rotate mode), so apply the grabbing cursor
    ...
    case 'rotate':
      {
        ...
        if (!this.ispt) {
          ...
          for (i = 0, f; f = this.selection_.item(i); i++) {
            ...
            geometry.rotate(a - this.angle_, this.center_); // bug: ol, bad calculation circle geom extent
    ...
    case 'translate':
      {
        ...
        var deltaY = evt.coordinate[1] - this.coordinate_[1]; //this.feature_.getGeometry().translate(deltaX, deltaY);
...
ol.interaction.Transform.prototype.handleMoveEvent_ = function (evt) {
  if (!this._handleEvent(evt, this.features_)) return; // console.log("handleMoveEvent");
...
ol.interaction.UndoRedo = function (options) {
  ...
  this._undoStack = [];
  this._redoStack = []; // Block counter

  this._block = 0; // Start recording

  this._record = true; // Custom definitions
...
ol.interaction.UndoRedo.prototype.setMap = function (map) {
  ol.interaction.Interaction.prototype.setMap.call(this, map); // Watch sources
...
ol.interaction.UndoRedo.prototype._watchSources = function () {
  var map = this.getMap(); // Clear listeners
  ...
  this._sourceListener = []; // Ges vector layers 
  ...
  if (map) {
    ...
    }.bind(this)); // Watch new inserted/removed
...
ol.interaction.UndoRedo.prototype._watchInteractions = function () {
  ...
  if (map) {
    ...
    map.getInteractions().forEach(function (i) {
      this._interactionListener.push(i.on(['setattributestart', 'modifystart', 'rotatestart', 'translatestart', 'scalestart', 'deletestart', 'deleteend', 'beforesplit', 'aftersplit'], this._onInteraction.bind(this)));
    }.bind(this)); // Watch new inserted / unwatch removed

    this._interactionListener.push(map.getInteractions().on(['add', 'remove'], this._watchInteractions.bind(this)));
  }
};
...
ol.interaction.UndoRedo.prototype._onInteraction.rotatestart = ol.interaction.UndoRedo.prototype._onInteraction.translatestart = ol.interaction.UndoRedo.prototype._onInteraction.scalestart = ol.interaction.UndoRedo.prototype._onInteraction.modifystart = function (e) {
  this.blockStart();
...
ol.interaction.UndoRedo.prototype._onInteraction.beforesplit = ol.interaction.UndoRedo.prototype._onInteraction.deletestart = ol.interaction.UndoRedo.prototype.blockStart;
/** End an undo block
 * @api
 */
...
ol.interaction.UndoRedo.prototype._onInteraction.aftersplit = ol.interaction.UndoRedo.prototype._onInteraction.deleteend = ol.interaction.UndoRedo.prototype.blockEnd;
/** handle undo/redo
 * @private
 */
...
ol.interaction.UndoRedo.prototype._handleDo = function (e, undo) {
  // Not active
  if (!this.getActive()) return; // Stop recording while undoing
  ...
  switch (e.type) {
    case 'addfeature':
      {
        if (undo) e.source.removeFeature(e.feature);else e.source.addFeature(e.feature);
        break;
      }

    case 'removefeature':
      {
        if (undo) e.source.addFeature(e.feature);else e.source.removeFeature(e.feature);
        break;
      }
    ...
    case 'changeattribute':
      {
        ...
        for (var p in oldp) {
          if (oldp === undefined) e.feature.unset(p);else e.feature.set(p, oldp[p]);
        }
    ...
    default:
      {
        if (this._defs[e.type]) {
          if (undo) this._defs[e.type].undo(e.prop);else this._defs[e.type].redo(e.prop);
        } else {
  ...
  if (this._block) {
    if (undo) this.undo();else this.redo();
  }

  this._record = true; // Dispatch event
...
ol.source.BinBase = function (options) {
  ...
  this._listen = options.listenChange !== false; // Geometry function
...
ol.source.BinBase.prototype._onRemoveFeature = function (e, bin, listen) {
  ...
  } else {// console.log("[ERROR:Bin] remove feature: feature doesn't exists anymore.");
  }
...
ol.source.BinBase.prototype._flatAttributes = function ()
/*bin, features*/
{};
/**
...
ol.source.DBPedia = function (opt_options) {
  ...
  if (!options.attributions) options.attributions = ["&copy; <a href='http://dbpedia.org/'>DBpedia</a> CC-by-SA"]; // Bbox strategy : reload at each move
...
ol.source.DBPedia.prototype.querySubject = function () {
  return "?subject rdfs:label ?label. " + "OPTIONAL {?subject dbpedia-owl:thumbnail ?thumbnail}." + "OPTIONAL {?subject dbpedia-owl:abstract ?abstract} . " + "OPTIONAL {?subject rdf:type ?type}";
};
...
ol.source.DBPedia.prototype.queryFilter = function () {
  return "lang(?label) = '" + this._lang + "' " + "&& lang(?abstract) = '" + this._lang + "'"; // Filter on type 
...
ol.source.DBPedia.prototype._loaderFn = function (extent, resolution, projection) {
  ...
  var self = this;
  var bbox = ol.proj.transformExtent(extent, projection, "EPSG:4326"); // SPARQL request: for more info @see http://fr.dbpedia.org/

  var query = "PREFIX geo: <http://www.w3.org/2003/01/geo/wgs84_pos#> " + "SELECT DISTINCT * WHERE { " + "?subject geo:lat ?lat . " + "?subject geo:long ?long . " + this.querySubject() + " . " + "FILTER(" + this.queryFilter() + ") . " // Filter bbox
  + "FILTER(xsd:float(?lat) <= " + bbox[3] + " && " + bbox[1] + " <= xsd:float(?lat) " + "&& xsd:float(?long) <= " + bbox[2] + " && " + bbox[0] + " <= xsd:float(?long) " + ") . " + "} LIMIT " + this._limit; // Ajax request to get the tile
...
(function () {
  ...
  */
  ...
  ol.style.dbPediaStyleFunction = function (options) {
    if (!options) options = {}; // Get font function using dbPedia type

    var getFont;

    switch (_typeof(options.glyph)) {
      case "function":
    ...
    var prefix = options.prefix ? options.prefix + "_" : ""; // Vector style function
...
ol.source.DFCI = function (options) {
  ...
  ol.source.Vector.call(this, options);
  this.set('resolutions', options.resolutions || [1000, 100, 20]); // Add Lambert IIe proj 
...
ol.source.DFCI.prototype._getFeatures = function (level, extent, projection) {
  ...
  if (level > 0) step /= 5;
  if (level > 1) step /= 10;
  var p0 = [Math.max(this._bbox[0][0], Math.floor(extent[0] / step) * step), Math.max(this._bbox[0][1], Math.floor(extent[1] / step) * step)];
  var p1 = [Math.min(this._bbox[1][0] + 99999, Math.floor(extent[2] / step) * step), Math.min(this._bbox[1][1] + 99999, Math.floor(extent[3] / step) * step)];

  for (var x = p0[0]; x <= p1[0]; x += step) {
    for (var y = p0[1]; y <= p1[1]; y += step) {
      var p,
          geom = [[x, y], [x + step, y], [x + step, y + step], [x, y + step], [x, y]];

      if (level > 2) {
        var m = this._midPt(geom[0], geom[2]); // .5
        ...
        features.push(this._trFeature(g, ol.coordinate.toDFCI([x, y], 2) + '.5', level, projection)); // .1 > .4
...
ol.source.Delaunay = function (options) {
  ...
  ol.source.Vector.call(this, options); // Convex hull

  this.hull = []; // A new node is added to the source node: calculate the new triangulation

  this._nodes.on('addfeature', this._onAddNode.bind(this)); // A new node is removed from the source node: calculate the new triangulation
...
ol.source.Delaunay.prototype._onRemoveNode = function (evt) {
  ...
  if (!pt) return; // Still there (when removing duplicated points)

  if (this.getNodesAt(pt).length) return; // Get associated triangles
  ...
  function testEdge(p0, p1, index) {
    if (ol.coordinate.equal(p0, pts[index])) {
      if (index) pts.push(p1);else pts.unshift(p1);
      return true;
    }
  ...
  while (true) {
    ...
    if (testEdge(e[0], e[1], 0) || testEdge(e[1], e[0], 0) || testEdge(e[0], e[1], pts.length - 1) || testEdge(e[1], e[0], pts.length - 1)) {
      edges.splice(i, 1);
      i = 0;
    } else {
      i++;
  ...
  if (closed) pts.pop(); // Update convex hull: remove pt + add new ones
  ...
  this.hull = ol.coordinate.convexHull(this.hull.concat(pts)); // select.getFeatures().clear();
  // 

  var clockwise = function clockwise(t) {
    var i1,
        s = 0;
  ...
  for (i = 0; i <= pts.length + 1; i++) {
    ...
    var t = [pts[i % pts.length], pts[(i + 1) % pts.length], pts[(i + 2) % pts.length]];
    ...
    if (clockwise(t) === clock) {
      ...
      if (ok) {
        ...
        this._addTriangle(t); // remove


        pts.splice((i + 1) % pts.length, 1); // and restart
...
ol.source.Delaunay.prototype._onAddNode = function (e) {
  ...
  this.flip = [];
  var nodes = this.getNodes(); // The point

  var pt = finserted.getGeometry().getCoordinates(); // Test existing point
  ...
  if (t) {
    ...
    hull2.push(pt);
    hull2 = ol.coordinate.convexHull(hull2); // Search for points
...
ol.source.Delaunay.prototype.flipTriangles = function () {
  ...
  while (this.flip.length) {
    ...
    for (var k = 0; k < 3; k++) {
      ...
      var pt1 = null; // Get opposite point
...
ol.source.Delaunay.prototype.intersectSegs = function (d1, d2) {
  ...
  if (det != 0) {
    var k = (d1x * d1[0][1] - d1x * d2[0][1] - d1y * d1[0][0] + d1y * d2[0][0]) / det; // Intersection: return [d2[0][0] + k*d2x, d2[0][1] + k*d2y];
...
ol.source.GeoImage = function (opt_options) {
  ...
  this.center = opt_options.imageCenter; // Scale of the image 

  this.scale = opt_options.imageScale; // Rotation of the image

  this.rotate = opt_options.imageRotate ? opt_options.imageRotate : 0; // Crop of the image

  this.crop = opt_options.imageCrop; // Mask of the image

  this.mask = opt_options.imageMask; // Load Image
  ...
  if (!opt_options.image) this._image.src = opt_options.url; // Draw image on canvas
  ...
  /*, projection*/
    ...
    canvas.height = size[1];
    var ctx = canvas.getContext('2d');
    if (!this._imageSize) return canvas; // transform coords to pixel

    function tr(xy) {
      return [(xy[0] - extent[0]) / (extent[2] - extent[0]) * size[0], (xy[1] - extent[3]) / (extent[1] - extent[3]) * size[1]];
    } // Clipping mask
  ...
  this.setCrop(this.crop); // Calculate extent on change
...
ol.source.GeoImage.prototype.calculateExtent = function (usemask) {
  ...
  if (usemask !== false && this.getMask()) {
    ...
    var extent = ol.extent.boundingExtent([[center[0] - width / 2, center[1] - height / 2], [center[0] + width / 2, center[1] + height / 2]]);
...
ol.source.Geoportail = function (layer, options) {
  ...
  ol.source.WMTS.call(this, wmts_options); // Load url using basic authentification
...
ol.source.Geoportail.prototype.getFeatureInfoUrl = function (coord, resolution, projection, options) {
  ...
  return url.replace(/Request=GetTile/i, 'Request=getFeatureInfo') + '&INFOFORMAT=' + (options.INFO_FORMAT || 'text/plain') + '&I=' + i + '&J=' + j;
};
/** Get feature info
 * @param {ol.Coordinate} coord 
 * @param {Number} resolution 
...
ol.source.Mapillary = function (opt_options) {
  ...
  if (!options.attributions) options.attributions = ["&copy; <a href='https://www.mapillary.com/'>Mapillary</a>"]; // Bbox strategy : reload at each move

  if (!options.strategy) options.strategy = ol.loadingstrategy.bbox; // Init parent

  ol.source.Vector.call(this, options); // Client ID
...
ol.source.Mapillary.prototype.readFeature = function ()
/*feature, attributes*/
{
...
ol.source.Mapillary.prototype._loaderFn = function (extent, resolution, projection) {
  if (resolution > this._maxResolution) return;
  var bbox = ol.proj.transformExtent(extent, projection, "EPSG:4326"); // Commons API: for more info @see https://www.mapillary.com/developer

  var date = Date.now() - 6 * 30 * 24 * 60 * 60 * 1000;
  var url = "https://a.mapillary.com/v2/search/im?client_id=" + this.get('clientId') + "&max_lat=" + bbox[3] + "&max_lon=" + bbox[2] + "&min_lat=" + bbox[1] + "&min_lon=" + bbox[0] + "&limit=" + (this._limit - 1) + "&start_time=" + date; // Ajax request to get the tile
...
ol.source.Overpass.prototype._loaderFn = function (extent, resolution, projection) {
  ...
  bbox = bbox[1] + ',' + bbox[0] + ',' + bbox[3] + ',' + bbox[2]; // Overpass QL
  ...
  ajax.onload = function () {
    ...
    var result = []; // Remove duplicated features
...
ol.layer.Vector3D = function (options) {
  ...
  ol.layer.Image.call(this, {
    source: new ol.source.ImageCanvas({
      canvasFunction: function canvasFunction(extent, resolution, pixelRatio, size
      /*, projection*/
      ) {
        canvas.width = size[0];
        canvas.height = size[1];
...
ol.layer.Vector3D.prototype.setStyle = function (s) {
  if (s instanceof ol.style.Style) this._style = s;else this._style = new ol.style.Style();
...
ol.layer.Vector3D.prototype.onPostcompose_ = function (e) {
  ...
  if (this.animate_) {
    ...
    if (elapsed < this.animateDuration_) {
      this.elapsedRatio_ = this.easing_(elapsed / this.animateDuration_); // tell OL3 to continue postcompose animation
  ...
  var ratio = e.frameState.pixelRatio;
  var ctx = e.context;
  var m = this.matrix_ = e.frameState.coordinateToPixelTransform; // Old version (matrix)

  if (!m) {
    m = e.frameState.coordinateToPixelMatrix, m[2] = m[4];
    m[3] = m[5];
    m[4] = m[12];
    m[5] = m[13];
  }

  this.center_ = [ctx.canvas.width * this.get('center')[0] / ratio, ctx.canvas.height * this.get('center')[1] / ratio];
...
ol.layer.Vector3D.prototype.animate = function (options) {
  ...
  this.easing_ = options.easing || ol.easing.easeOut; // Force redraw
...
ol.layer.Vector3D.prototype.hvector_ = function (pt, h) {
  var p0 = [pt[0] * this.matrix_[0] + pt[1] * this.matrix_[1] + this.matrix_[4], pt[0] * this.matrix_[2] + pt[1] * this.matrix_[3] + this.matrix_[5]];
  return {
    p0: p0,
    p1: [p0[0] + h / this.res_ * (p0[0] - this.center_[0]), p0[1] + h / this.res_ * (p0[1] - this.center_[1])]
  };
};
/**
*/
...
ol.source.WikiCommons = function (opt_options) {
  ...
  if (!options.attributions) options.attributions = ["&copy; <a href='https://commons.wikimedia.org/'>Wikimedia Commons</a>"]; // Bbox strategy : reload at each move
...
ol.source.WikiCommons.prototype._loaderFn = function (extent, resolution, projection) {
  if (resolution > this._maxResolution) return;
  var self = this;
  var bbox = ol.proj.transformExtent(extent, projection, "EPSG:4326"); // Commons API: for more info @see https://commons.wikimedia.org/wiki/Commons:API/MediaWiki

  var url = "https://commons.wikimedia.org/w/api.php?action=query&format=json&origin=*&prop=coordinates|imageinfo" + "&generator=geosearch&iiprop=timestamp|user|url|extmetadata|metadata|size&iiextmetadatafilter=LicenseShortName" + "&ggsbbox=" + bbox[3] + "|" + bbox[0] + "|" + bbox[1] + "|" + bbox[2] + "&ggslimit=" + this._limit + "&iilimit=" + (this._limit - 1) + "&ggsnamespace=6"; // Ajax request to get the tile
...
ol.layer.AnimatedCluster = function (opt_options) {
  ...
  this.set('animationMethod', options.animationMethod || ol.easing.easeOut); // Save cluster before change
...
ol.layer.Geoportail = function (layer, options, tileoptions) {
  ...
  if (!capabilities) {
    ...
    console.error("ol.layer.Geoportail: no layer definition for \"" + layer + "\"\nTry to use ol/layer/Geoportail~loadCapabilities() to get it."); // throw new Error("ol.layer.Geoportail: no layer definition for \""+layer+"\"");
  ...
  if (!options.extent && capabilities.bbox) {
    if (capabilities.bbox[0] > -170 && capabilities.bbox[2] < 170) options.extent = ol.proj.transformExtent(capabilities.bbox, 'EPSG:4326', 'EPSG:3857');
  ...
  this.set('queryable', capabilities.queryable); // BUG GPP: Attributions constraints are not set properly :(
...
ol.layer.Geoportail.getCapabilities = function (gppKey) {
  ...
  var geopresolutions = [156543.03390625, 78271.516953125, 39135.7584765625, 19567.87923828125, 9783.939619140625, 4891.9698095703125, 2445.9849047851562, 1222.9924523925781, 611.4962261962891, 305.74811309814453, 152.87405654907226, 76.43702827453613, 38.218514137268066, 19.109257068634033, 9.554628534317017, 4.777314267158508, 2.388657133579254, 1.194328566789627, 0.5971642833948135, 0.29858214169740677, 0.14929107084870338]; // Transform resolution to zoom
  ...
  function mergeConstraints(ori) {
    for (var i = ori.constraint.length - 1; i > 0; i--) {
      for (var j = 0; j < i; j++) {
        ...
        if (ori.constraint[i].maxZoom == ori.constraint[j].minZoom || ori.constraint[j].maxZoom == ori.constraint[i].minZoom || ori.constraint[i].maxZoom + 1 == ori.constraint[j].minZoom || ori.constraint[j].maxZoom + 1 == ori.constraint[i].minZoom || ori.constraint[i].minZoom - 1 == ori.constraint[j].maxZoom || ori.constraint[j].minZoom - 1 == ori.constraint[i].maxZoom) {
  ...
  ol.ext.Ajax.get({
    ...
    success: function success(resp) {
      ...
      for (var i = 0, l; l = layers[i]; i++) {
        // WMTS ?
        if (!/WMTS/.test(l.getElementsByTagName('Server')[0].attributes['service'].value)) continue; //        if (!all && !/geoportail\/wmts/.test(l.find("OnlineResource").attr("href"))) continue;
...
ol.source.Source.prototype.getPreview = function ()
/*lonlat, resolution*/
{
...
ol.layer.Base.prototype.getPreview = function (lonlat, resolution) {
  ...
  if (!resolution) resolution = 150; // Get middle resolution
      ...
      case "Point":
        {
          ...
          if (t) {
            ...
            ctx.fillRect(p[0] - m.width / 2 - 5, p[1] - h - 5, m.width + 10, h + 10);
            ctx.strokeRect(p[0] - m.width / 2 - 5, p[1] - h - 5, m.width + 10, h + 10);
            ctx.fillStyle = f; //console.log(build[i].feature.getProperties())
          }
...
ol.Overlay.Popup = function (options) {
  var self = this;
  options = options || {};
  if (typeof options.offsetBox === 'number') this.offsetBox = [options.offsetBox, options.offsetBox, options.offsetBox, options.offsetBox];else this.offsetBox = options.offsetBox; // Popup div

  var element = document.createElement("div"); //element.classList.add('ol-overlaycontainer-stopevent');

  options.element = element; // Anchor div

  var anchorElement = document.createElement("div");
  anchorElement.classList.add("anchor");
  element.appendChild(anchorElement); // Content
  ...
  this._elt = this.element; // call setPositioning first in constructor so getClassPositioning is called only once

  this.setPositioning(options.positioning || 'auto');
  this.setPopupClass(options.popupClass || options.className || 'default'); // Show popup on timeout (for animation purposes)
...
ol.Overlay.Popup.prototype.setClosebox = function (b) {
  this.closeBox = b;
  if (b) this._elt.classList.add("hasclosebox");else this._elt.classList.remove("hasclosebox");
};
/**
...
ol.Overlay.Popup.prototype.setPopupClass = function (c) {
  this._elt.className = "";
  var classesPositioning = this.getClassPositioning().split(' ').filter(function (className) {
    return className.length > 0;
  });
...
ol.Overlay.Popup.prototype.setPositioning_ = function (pos) {
  if (this._elt) {
    ...
    var classes = this.getClassPositioning().split(' ').filter(function (className) {
      return className.length > 0;
    });
...
ol.Overlay.Popup.prototype.show = function (coordinate, html) {
  ...
  if (html && html !== this.prevHTML) {
    ...
    Array.prototype.slice.call(this.content.querySelectorAll('img')).forEach(function (image) {
  ...
  if (coordinate) {
    ...
    this.setPosition(coordinate); // Set visible class (wait to compute the size/position first)
...
ol.Overlay.Placemark = function (options) {
  ...
  options.positioning = 'bottom-center', ol.Overlay.Popup.call(this, options);
...
ol.Overlay.Placemark.prototype.setClassName = function (name) {
  ...
  this.element.className = 'ol-popup placemark ol-popup-bottom ol-popup-center ' + (/visible/.test(oldclass) ? 'visible ' : '') + (/anim/.test(oldclass) ? 'anim ' : '') + name;
};
/**
...
ol.Overlay.PopupFeature = function (options) {
  ...
  this.set('maxChar', options.maxChar || 200); // Bind with a select interaction
...
ol.Overlay.PopupFeature.prototype.show = function (coordinate, features) {
  if (coordinate instanceof ol.Feature || coordinate instanceof Array && coordinate[0] instanceof ol.Feature) {
    features = coordinate;
    coordinate = null;
  }
  ...
  if (!this._count) this._count = 1; // Calculate html upon feaures attributes
...
ol.Overlay.PopupFeature.prototype._getHtml = function (feature) {
  ...
  var template = this._template; // calculate template
  ...
  if (template.attributes) {
    var tr,
        table = ol.ext.element.create('TABLE', {
      parent: html
    });
    ...
    for (var att in atts) {
      var a = atts[att];
      var content,
          val = feature.get(att); // Get calculated value
...
ol.Overlay.PopupFeature.prototype.setFix = function (fix) {
  if (fix) this.element.classList.add('ol-fixed');else this.element.classList.remove('ol-fixed');
};
/** Is a popup fixed
 * @return {boolean} 
 */
...
ol.Overlay.PopupFeature.localString = function (locales, options) {
  return function (a) {
    if (a && a.toLocaleString) {
      ...
      var date = new Date(a);
      if (isNaN(date)) return a;else return date.toLocaleString(locales, options);
    }
  };
};
/*	Copyright (c) 2018 Jean-Marc VIGLINO, 
...
ol.Overlay.Tooltip.prototype.getHTML = function (feature, info) {
  if (this.get('measure')) return this.get('measure') + (info ? '<br/>' + info : '');else return info || '';
};
...
ol.Overlay.Tooltip.prototype.setFeature = function (feature) {
  ...
  if (feature && feature.feature) feature = feature.feature; // The feature
...
(function () {
  ...
  */
  var clockwise = function clockwise(a, b, o) {
  ...
  var getCoordinates = function getCoordinates(geom) {
    var i, p;
    var h = [];
...
ol.coordinate.toDFCI = function (coord, level, projection) {
  ...
  var step = 100000;
  s += String.fromCharCode(65 + Math.floor((x < 800000 ? x : x + 200000) / step)) + String.fromCharCode(65 + Math.floor((y < 2300000 ? y : y + 200000) / step) - 1500000 / step);
  if (level === 0) return s; // Level 1

  var step1 = 100000 / 5;
  s += 2 * Math.floor(x % step / step1);
  s += 2 * Math.floor(y % step / step1);
  if (level === 1) return s; // Level 2

  var step2 = step1 / 10;
  var x0 = Math.floor(x % step1 / step2);
  s += String.fromCharCode(65 + (x0 < 8 ? x0 : x0 + 2));
  s += Math.floor(y % step1 / step2);
  if (level === 2) return s; // Level 3

  var x3 = Math.floor(x % step2 / 500);
  var y3 = Math.floor(y % step2 / 500);

  if (x3 < 1) {
    if (y3 > 1) s += '.1';else s += '.4';
  } else if (x3 > 2) {
    if (y3 > 1) s += '.2';else s += '.3';
  } else if (y3 > 2) {
    if (x3 < 2) s += '.1';else s += '.2';
  } else if (y3 < 1) {
    if (x3 < 2) s += '.4';else s += '.3';
  } else {
    s += '.5';
  }
...
ol.graph.Dijskra = function (options) {
  ...
  this.nodes = new ol.source.Vector(); // Maximum iterations

  this.maxIteration = options.maxIteration || 20000;
  this.stepIteration = options.stepIteration || 2000; // A* optimisation
...
ol.graph.Dijskra.prototype.weight = function ()
/* feature */
{
  return 1;
};
...
ol.graph.Dijskra.prototype.direction = function ()
/* feature */
{
  return 2;
};
...
ol.graph.Dijskra.prototype.addNode = function (p, wdist, dist, from, prev) {
  // Final condition
  if (this.wdist && wdist > this.wdist) return false; // Look for existing point

  var node = this.getNode(p); // Optimisation ?
  ...
  if (node) {
    // Allready there
    if (node !== this.arrival && node.get('wdist') <= wdist) return node; // New candidat
    ...
    } else this.candidat.push(node); // Add it in the node source
...
ol.graph.Dijskra.prototype.closestCoordinate = function (p) {
  ...
  if (ol.coordinate.dist2d(p, p0) < ol.coordinate.dist2d(p, p1)) return p0;else return p1;
};
...
ol.graph.Dijskra.prototype.path = function (start, end) {
  if (this.running) return false; // Starting nodes
  ...
  this.candidat = [];
  this.wdist = 0;
  this.running = true; // Starting point

  this.addNode(start, 0); // Arrival

  this.arrival = this.addNode(this.end, -1); // Start
...
ol.graph.Dijskra.prototype._resume = function () {
  ...
  while (this.candidat.length) {
    ...
    var p = node.getGeometry().getCoordinates(); // Find connected edges
    ...
    for (var i = 0, e; e = edges[i]; i++) {
      if (node.get('from') !== e) {
        ...
        if (dist < 0) {
          console.log('distance < 0!'); // continue;
        }
...
ol.coordinate.offsetCoords = function (coords, offset) {
  ...
  if (!isClosed) {
    p0 = coords[0];
    p1 = coords[1];
    p2 = [p0[0] + (p1[1] - p0[1]) / ol.coordinate.dist2d(p0, p1) * offset, p0[1] - (p1[0] - p0[0]) / ol.coordinate.dist2d(p0, p1) * offset];
    path.push(p2);
...
ol.geom.LineString.prototype.splitAt = function (pt, tol) {
  ...
  for (i = 0; i < c0.length - 1; i++) {
    // Filter equal points
    if (ol.coordinate.equal(c0[i], c0[i + 1])) continue; // Extremity found
...
ol.geom.MultiPolygon.prototype.scribbleFill = function (options) {
  ...
  if (!scribbles.length) return null; // Merge scribbles
...
ol.geom.Polygon.prototype.scribbleFill = function (options) {
  ...
  var i, k, l; // Geometry + rotate

  var geom = this.clone();
  geom.rotate(angle, [0, 0]);
  var coords = geom.getCoordinates(); // Merge holes

  var coord = coords[0];

  for (i = 1; i < coords.length; i++) {
    // Add a separator
    coord.push([]); // Add the hole

    coord = coord.concat(coords[i]);
  } // Extent 


  var ext = geom.getExtent(); // Split polygon with horizontal lines
  ...
  if (!lines.length) return null; // Order lines on segment index
    ...
    while (l) {
      ...
      while (lines[k]) {
        ...
        if (lines[k][0].pt[1] === nexty) {
          var d = Math.min((lines[k][0].index - l[0].index + mod) % mod, (l[0].index - lines[k][0].index + mod) % mod);
          var d2 = Math.min((l[1].index - l[0].index + mod) % mod, (l[0].index - l[1].index + mod) % mod);

          if (d < d0 && d < d2) {
            d0 = d;
            if (!lines[k][0].done) l2 = lines[k];else l2 = null;
          }
        }
...
ol.sphere.computeDestinationPoint = function (origin, distance, bearing, options) {
  ...
  var lon = lambda2 / toRad; // normalise to >=-180 and <=180° 
...
ol.Map.prototype.animExtent = function (extent, options) {
  var listenerKey;
  options = options || {}; // Change to map's projection
  ...
  var color = options.style ? options.style.getColr() || 'red' : 'red'; // Animate function

  function animate(event) {
    var frameState = event.frameState;
    var ratio = frameState.pixelRatio;
    var elapsed = frameState.time - start;
    if (elapsed > duration) ol.Observable.unByKey(listenerKey);else {
      ...
      context.beginPath(); // var e = easing(elapsedRatio)
      ...
      context.stroke();
      context.restore(); // tell OL3 to continue postcompose animation
...
ol.geom.LineString.prototype.calcCSpline_ = function (options) {
  ...
  pts = line.slice(0); // The algorithm require a previous and next point to the actual point array.
  ...
  for (i = 1; i < pts.length - 2; i++) {
    ...
    for (t = 0; t <= numOfSegments; t++) {
      ...
      y = c1 * pts[i][1] + c2 * pts[i + 1][1] + c3 * t1y + c4 * t2y; //store points in array
...
ol.HexGrid = function (options) {
  options = options || {};
  ol.Object.call(this, options); // Options
...
ol.InseeGrid = function (options) {
  options = options || {}; // Define EPSG:3035 if none
...
ol.Map.prototype.markup = function (coords, options) {
  ...
  if (!(style instanceof Array)) style = [style]; // Animate function
...
ol.ordering.zIndex = function (options) {
  ...
  if (options.equalFn) {
    ...
    return function (f0, f1) {
      if (f0.get(attr) == f1.get(attr)) return 0;else return f0.get(attr) < f1.get(attr) ? 1 : -1;
    };
  }
};
/*	Copyright (c) 2015 Jean-Marc VIGLINO, 
...
ol.style.Chart.prototype.renderChart_ = function () {
  ...
  var canvas = this.getImage(); // draw the circle on the canvas
  ...
  var step = this.animation_.animate ? this.animation_.step : 1; //console.log(this.animation_.step)
  // Draw pie
    ...
    default:
      {
        ...
        for (i = 0; i < this.data_.length; i++) {
          ...
          context.rect(x0, b - h, s, h); //console.log ( x0+", "+(b-this.data_[i]/max*2*this.radius_)+", "+x+", "+b);
...
ol.style.Chart.prototype.getChecksum = function () {
  ...
  var fillChecksum;
  var recalculate = this.checksums_ === null || strokeChecksum != this.checksums_[1] || fillChecksum != this.checksums_[2] || this.radius_ != this.checksums_[3] || this.data_.join('|') != this.checksums_[4];

  if (recalculate) {
    var checksum = 'c' + strokeChecksum + fillChecksum + (this.radius_ !== void 0 ? this.radius_.toString() : '-') + this.data_.join('|');
...
ol.style.FillPattern = function (options) {
  ...
  if (options.image) {
    ...
    if (pat.lines) for (i = 0; i < pat.lines.length; i++) {
...
ol.style.FillPattern.prototype.getPattern_ = function (options) {
  var pat = ol.style.FillPattern.prototype.patterns[options.pattern] || ol.style.FillPattern.prototype.patterns.dot;
...
ol.style.FontSymbol.prototype.renderMarker_ = function () {
  ...
  var canvas = this.getImage(); //console.log(this.getImage().width+" / "+(2 * (this.radius_ + strokeWidth) + 1));
  ...
  var context = canvas.getContext('2d');
  context.clearRect(0, 0, canvas.width, canvas.height);
  this.drawMarker_(renderOptions, context, 0, 0); // Set Anchor

  var a = this.getAnchor();
  a[0] = canvas.width / 2 - this.offset_[0];
  a[1] = canvas.width / 2 - this.offset_[1]; //this.createMarkerHitDetectionCanvas_(renderOptions);
};
/**
 * @private
...
ol.style.Photo = function (options) {
  ...
  ol.style.RegularShape.call(this, {
    ...
    points: 0 //	fill:new ol.style.Fill({color:"red"}) // No fill to create a hit detection Image
...
ol.style.Photo.prototype.renderPhoto_ = function () {
  ...
  var canvas = this.getImage(); // Draw hitdetection image

  var context = this.hitDetectionCanvas_.getContext('2d');
  this.drawBack_(context, "#000", strokeWidth);
  context.fill(); // Draw the image

  context = canvas.getContext('2d');
  this.drawBack_(context, strokeStyle, strokeWidth); // Draw a shadow
  ...
  img.src = this.src_; // Draw image

  if (img.width) self.drawImage_(img);else img.onload = function () {
    self.drawImage_(img); // Force change (?!)
    // self.setScale(1);
...
ol.style.Photo.prototype.drawImage_ = function (img) {
  var canvas = this.getImage(); // Remove the circle on the canvas
  ...
  var s, x, y, w, h, sx, sy, sw, sh; // Crop the image to a square vignette
  ...
  context.restore(); // Draw a circle to avoid aliasing on clip
...
ol.style.Photo.prototype.getChecksum = function () {
  var strokeChecksum = this.stroke_ !== null ? this.stroke_.getChecksum() : '-';
  var fillChecksum = this.fill_ !== null ? this.fill_.getChecksum() : '-';
  var recalculate = this.checksums_ === null || strokeChecksum != this.checksums_[1] || fillChecksum != this.checksums_[2] || this.radius_ != this.checksums_[3];

  if (recalculate) {
    var checksum = 'c' + strokeChecksum + fillChecksum + (this.radius_ !== void 0 ? this.radius_.toString() : '-');
...
(function () {
  ...
  */
  function drawTextPath(e) {
    ...
    var extent = e.frameState.extent;
    var c2p = e.frameState.coordinateToPixelTransform; // Get pixel path with coordinates
    ...
    for (var i = 0, f; f = features[i]; i++) {
      {
        ...
        for (var s, j = 0; s = style[j]; j++) {
          ...
          ctx.fillStyle = st.getFill() ? st.getFill().getColor() || "#000" : "#000"; // New params

          ctx.textJustify = st.getTextAlign() == "justify";
          ctx.textOverflow = st.getTextOverflow ? st.getTextOverflow() : "";
          ctx.minWidth = st.getMinWidth ? st.getMinWidth() : 0; // Draw textpath
  ...
  */
  ...
  ol.layer.Vector.prototype.setTextPathStyle = function (style, maxResolution) {
    ...
    if (typeof style == "function") this.textPathStyle_ = style;else this.textPathStyle_ = function () {
      return style;
    };
    this.textPathMaxResolution_ = Number(maxResolution) || Number.MAX_VALUE; // Force redraw
...
CanvasRenderingContext2D.prototype.textPath = function (text, path) {
  ...
  var nbspace = text.split(" ").length - 1; // Remove char for overflow
...
ol.style.Shadow.prototype.renderShadow_ = function () {
  ...
  s[1] = radius; // Remove the circle on the canvas
  ...
  context.closePath();
  context.fill();
  context.shadowColor = 'transparent'; // Set anchor
...
ol.style.StrokePattern.prototype.getPattern_ = function (options) {
  var pat = ol.style.FillPattern.prototype.patterns[options.pattern] || ol.style.FillPattern.prototype.patterns.dot;
  var d = Math.round(options.spacing) || 10;
  var size; //	var d2 = Math.round(d/2)+0.5;
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","dist/ol-ext.js"], null)
//# sourceMappingURL=/ol-ext.087003ae.js.map